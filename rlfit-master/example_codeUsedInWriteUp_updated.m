...
condition = 0;
plotPars.FontName = 'Arial';
plotPars.FontSize = 16;
plotPars.LineWidth       = 2.5;
plotPars.TickLength      = [.005 .005];

% set(gca,'LineWidth',plotPars.LineWidth/2,'TickLength',plotPars.TickLength);
% set(gca, 'TickLength' , plotPars.TickLength);
...
%             load([pars.dirs.saveDir filesep 'qDiffRel_ppData_model' num2str(modelNum) '_antic' num2str(pars.grabAnticResp) '_cond' num2str(condition) '_trinaryPupilMeasSetting' num2str(trinaryPupilMeasSetting) '.mat'],'ppData')
        ...
        %     title(['Beta correlates with avg pre-choice pupil diam.'])
            ylabel('Avg. Pupil Diameter','FontSize',12)
        ...
        %     figure
            ...
            for ppInd = 1:length(ppData.pupilDiams)
                ...
                pupilDataAcrossConds = pupilDiams(:,1); % blue state only
                exploreBooleans = ppData.exploratoryTrialsBoolean_diffToMaxMin{ppInd};
                exploreBooleans(exploreBooleans(:,1)==1,2)=-exploreBooleans(exploreBooleans(:,1)==1,2);
                pupilDiams= pupilDataAcrossConds;
...
%         saveas(gcf,[pars.dirs.figDir filesep 'paramCorrelations_beta_alpha_exploitCt_pupilAvg_trinaryPupilMeasSetting' num2str(trinaryPupilMeasSetting) '_cond' num2str(condition) '_modelFreeVsModelBasedAvgPupilSize.png'])
        ...
        for ppInd = 1:length(ppData.pupilDiams)
            ...
            allOutcomes(:,ppInd)=outcomes;
            states = two_stage_task_data.state;
            allStates(:,ppInd)=[states;states];
...
%             ppData.bCumSum{ppInd}=bCumSum;
            ...
            if qDiffs(find(exploreBooleans==1)) >= 0
                qDiffs(find(exploreBooleans==1))=-qDiffs(find(exploreBooleans==1));
            end
            exploreBooleans=(exploreBooleans(:,1));
            allExploreBooleans(ppInd,:)=exploreBooleans;
            pupilDiams = ppData.pupilDiams{ppInd};
            ...
            explorePupilDiams = pupilDiams(find(exploreBooleans==1));
...
%             cumSumsCollapsed=cumSumsCollapsed(find(exploreBooleans==exploitOrExplore));
            ...
            paramValsVsDiam(ppInd,13) = nanmean(explorePupilDiams);
            
        end
        excludedPps=[30,31];
        paramValsVsDiam(excludedPps,:) = nan;
        allOutcomes(:,excludedPps)=nan;
        allStates(:,excludedPps)=nan;
        accAllPp=cell2mat(ppData.acc);
        accAllPp=[accAllPp(1,1:29),nan,nan,accAllPp(1,30:end)];
        accAllPp(:,excludedPps)=nan;
        excludedPps = find(accAllPp < 0);

        accAllPpVar=accAllPp';
        accAllPpVar(excludedPps,:) = nan;
        outcomesVar=nanmean(allOutcomes,1)';
        outcomesVar(excludedPps,:) = nan;
        allStatesVar=nanmean(allStates,1)';
        allStatesVar(excludedPps,:) = nan;
        paramValsVsDiam2=paramValsVsDiam;
        paramValsVsDiam2(excludedPps,:) = nan;
        tbl_avgVals = table(paramValsVsDiam2(:,1),paramValsVsDiam2(:,2),...
            paramValsVsDiam2(:,3),paramValsVsDiam2(:,4),paramValsVsDiam2(:,5),...
            paramValsVsDiam2(:,6),paramValsVsDiam2(:,7),paramValsVsDiam2(:,8),...
            paramValsVsDiam2(:,9),paramValsVsDiam2(:,10),paramValsVsDiam2(:,11),...
            outcomesVar,allStatesVar,accAllPp',paramValsVsDiam(:,13),....
            'VariableNames',{...
            'wVal','avgDiam','meanQdiff','meanExploreBooleans','cumSumsCollapsed',...
            'sumExploreBooleans','meanExploitDiam','beta','lambda','alphaVal',...
            'exploitAfterExplorePupilDiams','avgOutcome','avgState','accAllPp','meanExploreDiam'});
%         tbl3.allExploreBooleans = nominal(tbl3.allExploreBooleans);
%         tbl_avgVals.avgState = nominal(tbl_avgVals.avgState);
        
% % %         lm = fitlm(tbl_avgVals,['avgDiam~(wVal*meanQdiff*meanExploreBooleans*(beta+alphaVal))'])% ...
% % %         lm = fitlm(tbl_avgVals,['avgDiam~meanExploreBooleans*(alphaVal+beta+lambda)'])% ...
% % % 
% % %         lm = fitlm(tbl_avgVals,['avgDiam~(meanExploreBooleans*wVal*lambda*meanQdiff + meanExploreBooleans*wVal*alphaVal*meanQdiff + meanExploreBooleans*wVal*beta*meanQdiff + (cumSumsCollapsed+beta+alphaVal+avgState))'])% ...
% % %         
        %% predict explore/exploit pupil diams (jk this is dumb I need repeated measures analysis here
%         lm_exploit = fitlm(tbl_avgVals,['meanExploitDiam~accAllPp*( wVal*sumExploreBooleans*(alphaVal+beta+meanQdiff+avgState))'])
%         lm_explore = fitlm(tbl_avgVals,['meanExploreDiam~accAllPp*( wVal*sumExploreBooleans*(alphaVal+beta+meanQdiff+avgState))'])
%         figure
%         subtightplot(1,2,1)
%         plotInteraction(lm_exploit,'wVal','beta','predictions')
%         subtightplot(1,2,2)
%         plotInteraction(lm_explore,'wVal','beta','predictions')
%         figure
%         subtightplot(1,2,1)
%         plotInteraction(lm_exploit,'wVal','alphaVal','predictions')
%         subtightplot(1,2,2)
%         plotInteraction(lm_explore,'wVal','alphaVal','predictions')
%         figure
%         subtightplot(1,2,1)
%         plotAdjustedResponse(lm_exploit,'wVal')
%         subtightplot(1,2,2)
%         plotAdjustedResponse(lm_explore,'wVal')
        
        %% .00866
        lm = fitlm(tbl_avgVals,['avgDiam~accAllPp*( wVal*sumExploreBooleans*(alphaVal+beta+meanQdiff+avgState))'])
%         lm = fitlm(tbl_avgVals,['avgDiam~accAllPp*( wVal*sumExploreBooleans*(alphaVal+beta+meanQdiff+avgOutcome))'])
        % rm'd bad accuracy pp's
% % %         lm = fitlm(tbl_avgVals,['avgDiam~( wVal*sumExploreBooleans*(alphaVal+beta+meanQdiff+avgOutcome))'])
% % % 
% % %         %% .0284
% % %         lm = fitlm(tbl_avgVals,['avgDiam~accAllPp*( wVal*sumExploreBooleans*(alphaVal+beta+meanQdiff+avgOutcome))'])
% % %         %% .122
% % %         lm = fitlm(tbl_avgVals,['avgDiam~accAllPp*( wVal*sumExploreBooleans*(alphaVal+beta))'])
% % %         %% 
% % %         lm = fitlm(tbl_avgVals,['avgDiam~accAllPp*( wVal*sumExploreBooleans + (alphaVal+beta+))'])
% % %         %% new boi, .048
% % %         lm = fitlm(tbl_avgVals,['avgDiam~accAllPp*( meanQdiff*wVal*(alphaVal+beta))'])

        figure
        plotAdjustedResponse(lm,'alphaVal')
        figure
        plotAdjustedResponse(lm,'beta')
        figure
        plotAdjustedResponse(lm,'accAllPp')
        figure
        plotAdjustedResponse(lm,'wVal')
        figure
        plotEffects(lm)
        figure
        plot(lm)
        %% larger learning rates imply decreased pupil size in model-based learners (strong trend)
        figure
        plotInteraction(lm,'wVal','alphaVal','predictions')
        %% Larger propensity to exploit rather than explore trends towards LARGER avg pupil size in model-based learners
        % I can argue that this might be due to the fact that less memory
        % processing has to occur across trials in order to consolidate
        % info
        figure
        plotInteraction(lm,'wVal','beta','predictions')
        %% More exploring across trials leads to decreased avg. pupil size
        % I could argue here that exposure to more novel stimuli demands
        % more memory consolidation/reorganizing, and SWRs. In fact, my
        % previous analysis found increased pupil size during exploration
        % trials, so this story fits
        figure
        plotInteraction(lm,'wVal','sumExploreBooleans','predictions')
        %% Hardly explore --> increasing beta leads to larger pupil-szies; explore lots--> recollecting info/increasing beta/increased exploitation predicts smaller pupil size
        % both of these seem to have the same relationship, whereby
        % increased exploration across trials leads to smaller pupil sizes
        % w/ increasing beta (alpha), and overall decreased exploration leads to larger
        % pupil sizes w/ increasing beta (alpha)
        
        % for model-based learners
        figure
        plotInteraction(lm,'sumExploreBooleans','beta','predictions')
        figure
        plotInteraction(lm,'sumExploreBooleans','alphaVal','predictions')

        %% This one makes sense
        % Increasing learning rate leads to smaller pupil sizes for
        % model-based learners, and vice versa for model-free learners.
        % This should occur if memory-processing acts as a precursor to
        % learned state-associations.
        figure
        plotInteraction(lm,'wVal','alphaVal','predictions')
 
        %% state luminance effect
        % this one might not matter at all
        figure
        plotInteraction(lm,'sumExploreBooleans','avgState','predictions') 
...
%         allWvals=allWvals(allWvals<thresh);
        ...
        Meas
        pupilData=allPupilDiams(allExploreBooleans);
        wData=allWvals(allExploreBooleans);
        Meas = table([1]','VariableNames',{'Measurements'});

        rm2 = fitrm(tbl3,'allWvals~allExploreBooleans*allPupilDiams*(allOutcomes+allStates+allAlphas+allBetas+allQdiffs)','WithinDesign',Meas)
        rm2 = fitrm(tbl3,'allPupilDiams~allExploreBooleans*allWvals*(allOutcomes+allStates+allAlphas+allBetas+allQdiffs+IDs)','WithinDesign',Meas)

        statstbl = grpstats(rm2,{'allExploreBooleans'})
        anovatbl = anova(rm) 
% % %         
% % %         rm = fitrm(tbl3,'allPupilDiams~ID','WithinDesign',Meas)
        
% % %         lm2 = fitlm(tbl3,['allPupilDiams~IDs+((allOutcomes*allStates+allOutcomes*allWvals+allQdiffs+allCumSums+allAlphas*allWvals+allBetas*allWvals+allLambdas*allWvals)*allExploreBooleans)'])% ...
% % %         lm2 = fitlm(tbl3,['allPupilDiams~IDs+((allOutcomes*allStates+allOutcomes*allWvals+allQdiffs+allCumSums+allAlphas*allWvals+allBetas*allWvals+allLambdas*allWvals)*allExploreBooleans)'])% ...
        lm2 = fitlm(tbl3,['allPupilDiams~IDs+((allOutcomes*allStates+allOutcomes+allQdiffs+allCumSums)*allExploreBooleans)'])% ...
...
%         saveas(gcf,[pars.dirs.figDir filesep 'linearModel_wInteraction' '_model_' num2str(modelNum) '_trinaryPupilMeasSetting' num2str(trinaryPupilMeasSetting) '_cond' num2str(condition) '_modelFreeVsModelBasedAvgPupilSize.png'])
        ...
        set(gca,'LineWidth',plotPars.LineWidth/2,'TickLength',plotPars.TickLength);
        set(gca, 'TickLength' , plotPars.TickLength);
        xlabel('Binned ''w'' estimates')
        ylabel('Bin ct (# of participants)')
        set(gca,'fontsize',18,'fontname',plotPars.FontName)
        ...
        gatherStimOnset='nan';
        saveas(gcf,[pars.dirs.figDir filesep 'MF-MB-pp-distributione_cond' num2str(condition) '_modelFreeVsModelBasedAvgPupilSize.png'])
        savefig(gcf,[pars.dirs.figDir filesep 'MF-MB-pp-distributione_cond' num2str(condition) '_modelFreeVsModelBasedAvgPupilSize.fig'])
    ...
    %     plot(lm)
        ...
        % titleAppend='model-based';
        % saveas(gcf,[pars.dirs.figDir filesep titleAppend '_stimOnsetBool' num2str(gatherStimOnset) '_meas' num2str(measOpts) '_cond' num2str(condition) '_modelFreeVsModelBasedAvgPupilSize.png'])
        saveas(gcf,[pars.dirs.figDir filesep 'MF_BM_Qdifs_v_pupilSizes_cond' num2str(condition) '_modelFreeVsModelBasedAvgPupilSize.png'])
        ...
        for ppType = 1:2
            ...
            pps = pps(pps ~= 30 & pps ~= 31);
            allExplorePupDiams = [];
            allExploitPupDiams = [];
            for modelPp = pps
                exploreBools=cell2mat(ppData.exploratoryTrialsBoolean_diffToMaxMin(:,1));
                exploreBools=exploreBools(:,1);
                exploreBools=[exploreBools,exploreBools,exploreBools];
                pupilData=ppData.pupilDiams{modelPp};
                if useMeanAcrossTrials
                    if isempty(allExplorePupDiams)
                        allExplorePupDiams = mean(pupilData(exploreBools==1),'omitnan');
                        allExploitPupDiams = mean(pupilData(exploreBools==0),'omitnan');
                    else
                        allExplorePupDiams = [allExplorePupDiams;mean(pupilData(exploreBools==1),'omitnan')];
                        allExploitPupDiams = [allExploitPupDiams;mean(pupilData(exploreBools==0),'omitnan')];
                    end
                else
                    if isempty(allExplorePupDiams)
                        allExplorePupDiams = (ppData.pupilDiams{modelPp});
                    else
                        allExplorePupDiams = [allExplorePupDiams;(ppData.pupilDiams{modelPp})];
                    end
                end
            end
        %     avgs = [];
        ...
        %     end
            if ppType == 1
                avgs_mf_explore = (allExplorePupDiams);
                avgs_mf_exploit = (allExploitPupDiams);
            else
                avgs_mb_explore = (allExplorePupDiams);
                avgs_mb_exploit = (allExploitPupDiams);
            end
        end
    %     dataForMeanComparisons
    ...
    %     [h,p,ci,stats] = ttest2(datas{2,1}(:),datas{2,2}(:))
        ...
        figure
        bar([1,2],[mean(avgs_mf_explore(:,1),'omitnan'),mean(avgs_mb_explore(:,1),'omitnan')])

        meanMF=mean(avgs_mf_explore(:,1),'omitnan')
        stdMF=std(avgs_mf_explore(:,1),'omitnan')
        meanMB=mean(avgs_mb_explore(:,1),'omitnan')
        stdMB=std(avgs_mb_explore(:,1),'omitnan')
        ...
        figure
        subtightplot(2,1,1)
        hist(avgs_mf_explore(:,1))
        xlim([0,4000])
        subtightplot(2,1,2)
        hist(avgs_mb_explore(:,1))
        xlim([0,4000])
        ...
        [h,p,ci,stats] = ttest2(avgs_mf_explore(:,1),avgs_mb_explore(:,1))
