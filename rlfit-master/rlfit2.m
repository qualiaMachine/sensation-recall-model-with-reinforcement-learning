function [beta, LL, Q] = rlfit2(Qfun, action1,action2,states,payoff,attentionWeights, lb, ub, niter)
...
LLfun = @(x, action1, action2,states,z) LL_softmax(x(1)*Qfun(x(1:end),action1,action2,z,states,attentionWeights), action1);

% then define a function to be minimized (the total negative log
% likelihood)
fitfun = @(beta)(-1)*sum(LLfun(beta, action1,action2,states, z));

% now combine upper and lower bounds on softmax temp with upper and lower
% bounds on other parameters
lb = [1e-5, lb]; %lower bounds
ub = [500, ub]; %upper bounds
...
% get action values
% keyboard
Q = Qfun(beta(1:end), action1,action2, z,attentionWeights);
