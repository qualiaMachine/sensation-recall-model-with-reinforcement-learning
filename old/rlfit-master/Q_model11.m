function prA=Q_model11(modelParams,action1,action2,payoff,states,pupilSizes)
...
memoryNoise = modelParams(5);
memoryNoise2 = modelParams(6);
memoryNoise3 = modelParams(7);
...
%     end
    ...
    origT=t;
    if t < 51
        tempMemoryVar = memoryNoise;
    elseif t < 151
        tempMemoryVar = memoryNoise2;
    else
        tempMemoryVar = memoryNoise3;
    end
    alpha = (alphaOrig)*.5 + (tempMemoryVar)*.5;
    alpha2 = alpha; %learning rate
...
%     end
    ...
    for t = tInds
        if t == 1
            ...
            P2A(t) = P2A(currT-1);% + memoryNoise*pupilSizes(t); %taking empirical probabilities from the previous trial
            P2A(P2A<0) = 0;
            P2A(P2A>1) = 1;
            P2B(t) = P2B(currT-1);% + memoryNoise*pupilSizes(t); 
            P2B(P2B<0) = 0;
            P2B(P2B>1) = 1;
    ...
    %     QB_MB(t) = recallWeights(t)*P2B(t)*max(QA_TD(t,2),QB_TD(t,2)) + (1-(recallWeights(t)*P2B(t)))*max(QA_TD(t,3),QB_TD(t,3));
        ...
        if (action1(t) == 0) 
            if (states(t) == 2)
                i = 2;
                if tempMemoryVar < inf%pupilSizes(t)
                    N(1,1) = N(1,1) + 2;
                else
                    N(1,1) = N(1,1) + 0;
                end
