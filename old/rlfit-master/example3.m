...
%     ub = [1,3500,1];%,3500,1,1]; %upper bounds
    lb = [0,0];%,1000,0];%,1000,0,0]; %lower bounds
    ub = [1,1];%,3500,1];%,3500,1,1]; %upper bounds
    ...
    outcome =  two_stage_task_data.payoff;

    % now optmize to fit model
    numiter = 15;
    [beta, LL, Q] = rlfit(Qfun, action1, outcome,attentionWeights,lb, ub, numiter);
...
%% Calc BIC values
BIC = (2*log(length(Q))*length(ppData.params{ppInd})) - (2*LL)
