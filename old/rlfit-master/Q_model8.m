function prA=Q_model8(modelParams,action1,action2,payoff,states,pupilSizes)
...
%     lambda=lambdaOrig*attentionWeights(t);
    if pupilSizes(t) < (memoryBooleanThresh*w) && ~isempty(memoryBuffer)
        tInds = [];
        if length(memoryBuffer) >=5
            maxInds = 5;
        else
...
%                 tInds = [tInds, randi([1 length(memoryBuffer)],1)];
        ...
        if length(memoryBuffer) > 20
            memoryBuffer=memoryBuffer(end-20:end); % keep most recent 15 timesteps
        end
    else
        tInds = t;
    end
